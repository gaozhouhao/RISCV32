...
enum {
  ...
  TYPE_N, TYPE_R, TYPE_J, TYPE_B, TYPE_SH, TYPE_CSRI// none
};
...
#define immSH() do { *imm = BITS(i, 24, 20); } while(0);
#define immCSRI() do { *imm = BITS(i, 31, 20); } while(0)
...
static void decode_operand(Decode *s, int *rd, word_t *src1, word_t *src2, word_t *imm, int type) {
  ...
  *rd     = BITS(i, 11, 7);
  switch (type) {
    ...
    case TYPE_CSRI:src1R();        immCSRI();break;
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai , SH, R(rd) = (int32_t)src1 >> imm);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr , I, R(rd) = s->pc + 4; 
                                                              ...
                                                              if(rd == 1) { // call
                                                                ...
                                                                Log("0x%08x:\tcall [%s@0x%08x]", s->pc, find_func(s->dnpc), s->dnpc);
                                                                // call_depth ++;
                                                              }
                                                              if(rd == 0 && rs1 == 1 && imm == 0) { // ret
                                                                ...
                                                                // for (int i = 0; i < call_depth; i ++) printf("  ");
                                                                // printf("ret [%s]\n", find_func(s->dnpc));
                                                                Log("0x%08x:\tret [%s]", s->pc, find_func(s->dnpc));
                                                              }););
  ...
  INSTPAT("??????? ????? ????? 001 ????? 11100 11", csrrw, CSRI, {if(rd !=0) R(rd) = cpu.csr[imm];cpu.csr[imm] = src1;});
  INSTPAT("??????? ????? ????? 010 ????? 11100 11", csrrs, CSRI, {R(rd) = cpu.csr[imm]; if(src1 != 0)cpu.csr[imm] |= src1;});
